...
struct equi {
  ...
  struct htlayout {
    ...
    u32 getxhash0(const htunit* slot) const {
      ...
      return (slot->bytes[prevbo] & 0xf) << 4 | slot->bytes[prevbo+1] >> 4;4
...
#error not implemented
           ...
           * For 210,9 - RESTBITS 7, nexthtunits = 7 = 28 bytes
           * hashsize = 25
           * When placing the hashes 28-25 = 3 which results in the 0 leading zeros
           * on words in round 1.
           *
           */
//          htunit *s = hta.heap0[bucketid][slot] + htl.nexthtunits;
          ...
          memcpy(s->bytes-hashbytes, ph+WN/8-hashbytes, hashbytes); //ORIGINAL
          //memcpy(s->bytes-hashbytes, ph+1, hashbytes);

          // round 0 tags store hash-generating index
          s->tag = tree((block * NBLAKES + i) * HASHESPERBLAKE + j);

          /*
          if(ph[1] == 0x5 && ph[2] == 0x92 && ph[3] == 0x94){
  ...
  void digitodd(const u32 r, const u32 id) {
    ...
    for (u32 bucketid=id; bucketid < NBUCKETS; bucketid += nthreads) {
      ...
      for (u32 s1 = 0; s1 < bsize; s1++) {   // loop over slots
        ...
        for (; cd.nextcollision(); ) {
          ...
          printf("prevbo: %d\n", htl.prevbo);
          printf("Buckmask: %d\n", BUCKMASK);
          printf("XorBucketId (Specific) %d\n", htobe32(slot0->word ^ slot1->word) >> 10 & BUCKMASK);
    	  printf("bytes size: %d\n", sizeof(bytes0)/sizeof(bytes0[0]));
	...
	*/
          ...
          xorbucketid = (u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) >> 4;
#elif WN == 210 && BUCKBITS == 12 && RESTBITS == 9
          //xorbucketid = (((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x1f) << 7)
          //                   | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 1;
          //printf("XOR BucketID: %d\n", xorbucketid);;
          xorbucketid = (((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0xf) << 8)
                             | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]);
#elif WN == 210 && BUCKBITS == 13 && RESTBITS == 8
	  ...
	  printf("prevbo: %d\n", htl.prevbo);
      printf("Buckmask: %d\n", BUCKMASK);
	  printf("XorBucketId (Specific) %d\n", htobe32(slot0->word ^ slot1->word) >> 8 & BUCKMASK);
	  printf("bytes size: %d\n", sizeof(bytes0)/sizeof(bytes0[0]));
	  ...
	  printf("%d\n", bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]);
	  */
      //xorbucketid = (((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x1f) << 8)
      //                   | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]);
      xorbucketid = ((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) << 5)
                               | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 3;
      //printf("XorBucketId (General) %d\n", xorbucketid);
      //printf("\n");
      //exit(0);

#elif WN == 220 && BUCKBITS == 14 && RESTBITS == 8
      xorbucketid = (((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x3f) << 8)
                         | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]);
#elif WN == 210 && BUCKBITS == 17 && RESTBITS == 4
      xorbucketid = ((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) << 8)
                          | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2])) << 1)
                          | (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 7;
	  ...
	  printf("1: %d\n", (bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x7);
	  printf("2: %d\n", (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]));
	  printf("3: %d\n", (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 5);
	  */

      xorbucketid =((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x7) << 8)
              | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2])) << 3
              | (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 5;

      //printf("xorbucketid: %d\n", xorbucketid);
      //exit(0);
#elif WN == 210 && BUCKBITS == 18 && RESTBITS == 3

      //Temporary
      xorbucketid = ((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x7 << 8)
                                | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2])) << 7)
                                | (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 1;
#elif WN == 210 && BUCKBITS == 16 && RESTBITS == 5

      //Temporary
      xorbucketid = ((((u32)(bytes0[htl.prevbo] ^ bytes1[htl.prevbo]) & 0x7 << 8)
                                | (bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1])) << 7)
                                | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 1;
#else
#error not implemented
#endif
    ...
    for (u32 bucketid=id; bucketid < NBUCKETS; bucketid += nthreads) {
      ...
      for (u32 s1 = 0; s1 < bsize; s1++) {
        ...
        for (; cd.nextcollision(); ) {
          ...
          xorbucketid = (u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) >> 4;
#elif WN == 210 && BUCKBITS == 12 && RESTBITS == 9
          xorbucketid = ((u32)(bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) << 4)
                            | (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 4;
#elif WN == 210 && BUCKBITS == 13 && RESTBITS == 8
          xorbucketid = ((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) << 5)
                            | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 3;
...
#error not implemented
  ...
  void digit1(const u32 id) {
    ...
    for (u32 bucketid=id; bucketid < NBUCKETS; bucketid += nthreads) {
      ...
      for (u32 s1 = 0; s1 < bsize; s1++) {
        ...
        /*
        printf("word: %d\n", slot1->word);
        printf("getxhash0 specialized : %d\n", htobe32(slot1->word) >> 20 & 0x3ff);
        for(int i = 0; i < 8; i++){
        	printf("%x ", slot1->bytes[i]);
        }
        printf("\n");
        exit(0);*/
        ...
        for (; cd.nextcollision(); ) {
          ...
          printf("\n");
          for(int i = 0; i < 24; i++){
        	  printf("%.2x ", slot0->bytes[i]);
          }
          printf("\n");
          for(int i = 0; i < 24; i++){
        	  printf("%.2x ", slot1->bytes[i]);
          }
          printf("\n");
		  */
          ...
          *x++ = x0[2] ^ x1[2];
          ...
          /*
          for(int i = 0; i < 24; i++){
        	  printf("%.2x ",heaps.heap1[xorbucketid][xorslot]->bytes[i]);
          }
          printf("\n");
          exit(0);
          */
  ...
  void digit2(const u32 id) {
    ...
    for (u32 bucketid=id; bucketid < NBUCKETS; bucketid += nthreads) {
      ...
      for (u32 s1 = 0; s1 < bsize; s1++) {
        ...
        if(slot1->bytes[0] == 0 && slot1->bytes[1] == 0xf && slot1->bytes[2] == 0x41 && slot1->bytes[3] == 0x76){
            for(int i = 0; i < 24; i++){
            	printf("%.2x ", slot1->bytes[i]);
            }
            printf("\n");
        ...
        */
        ...
        for (; cd.nextcollision(); ) {
          ...
          *x++ = x0[2] ^ x1[2];
          ...
          if(slot1->bytes[0] == 0 && slot1->bytes[1] == 0xf && slot1->bytes[2] == 0x41 && slot1->bytes[3] == 0x76){
			   for(int i = 0; i < 24; i++){
				  //printf("%.2x ",xs->bytes[i]);
				  printf("%.2x ",heaps.heap0[xorbucketid][xorslot]->bytes[i]);
			   }
			   printf("\n");
			   exit(0);
          }
  ...
  void digit3(const u32 id) {
    ...
    for (u32 bucketid=id; bucketid < NBUCKETS; bucketid += nthreads) {
      ...
      for (u32 s1 = 0; s1 < bsize; s1++) {
        ...
        if(slot1->bytes[0] == 0x44 && slot1->bytes[1] == 0x95){
		   for(int i = 0; i < 8; i++){
			printf("%x ", slot1->bytes[i]);
		   }
		   printf("\n");
	   ...
	   //exit(0);
        ...
        for (; cd.nextcollision(); ) {
          ...
          if(slot1->bytes[0] == 0x44 && slot1->bytes[1] == 0x95 && slot1->bytes[2] == 0xb2 && slot1->bytes[3] == 0xc4){
		   printf("\n");
		   for(int i = 0; i < 20; i++){
			  printf("%.2x ", slot0->bytes[i]);
		   }
		...
		//printf("In slot loop digit odd");
		  ...
		  for (; cd.nextcollision(); ) {
			...
			*/
			...
			/*
			for(int j = 0; j < 16; j++){
				printf("%x ", slot0->bytes[j]);
			}
			printf("\n");
			for(int j = 0; j < 16; j++){
				printf("%x ", slot1->bytes[j]);
			}
			printf("\n");
			*/
			for (u32 i=htl.dunits; i < htl.prevhtunits; i++){
				//printf("%u ", slot0[i].word ^ slot1[i].word);
			  xs++->word = slot0[i].word ^ slot1[i].word;
			}
			// store tree node right after hash
			xs->tag = tree(bucketid, s0, s1);
        ...
        */
        ...
        for (; cd.nextcollision(); ) {
		  ...
		  */
          ...
          htunit *xs = htl.hta.heap0[xorbucketid][xorslot];
          for (u32 i=htl.dunits; i < htl.prevhtunits; i++)
          //for (u32 i=0; i < htl.prevhtunits; i++)
        	  xs++->word = slot0[i].word ^ slot1[i].word;
          xs->tag = tree(bucketid, s0, s1);
        }
      }
		...
		//printf("In slot loop digit odd");
		  ...
		  for (; cd.nextcollision(); ) {
			...
			for(int i = 0; i < 8; i++){
			  ...
			  printf("\n");
			  */
			  //printf("1: %d\n", (bytes0[htl.prevbo] ^ bytes1[htl.prevbo]) & 0x1);
			  //printf("2: %d\n", (bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]));
			  //printf("3: %d\n", (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 3);


			xorbucketid =((((u32)(bytes0[htl.prevbo] ^ bytes1[htl.prevbo]) & 0x1) << 8)
					| (bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1])) << 5
					| (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 3;
  ...
  void digit1(const u32 id) {
        ...
        for (u32 s1 = 0; s1 < bsize; s1++) {
  		...
  		*/
          ...
          for (; cd.nextcollision(); ) {
  		  ...
  		  printf("3: %d\n", (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 5);
  		  */
  		  xorbucketid =((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x3f) << 7)
  				| ((bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 1) );
            ...
            if(htl.hta.heap0[xorbucketid][xorslot]->bytes[0] == 0x1 && htl.hta.heap0[xorbucketid][xorslot]->bytes[1] == 0x69 && htl.hta.heap0[xorbucketid][xorslot]->bytes[2] == 0x1f && htl.hta.heap0[xorbucketid][xorslot]->bytes[3] == 0x50 && htl.hta.heap0[xorbucketid][xorslot]->bytes[4] == 0x60){
            	for(int i = 0; i < 24; i++){
            		printf("%.2x ", slot0->bytes[i]);
            	}
            	printf("\n");
    ...
    void digit3(const u32 id) {
        ...
        /*
        for(int i = id; i < NBUCKETS; i++){
            slot0 *buck = htl.hta.heap0[i]; // point to first slot of this bucket
        	u32 bsize   = getnslots0(i);    // grab and reset bucket size
        	for (u32 s1 = 0; s1 < bsize; s1++) {   // loop over slots
        		const htunit *slot1 = buck[s1];
        		 for(int i=htl.prevbo; i < 16 + htl.prevbo; i++){
					printf("%.2x ", slot1->bytes[i]);
				}
        		 printf("\n");
        	}
        ...
        for (u32 bucketid=id; bucketid < NBUCKETS; bucketid += nthreads) {
    	  ...
    	  */
          ...
          //                   | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]);
              ...
              xorbucketid = ((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x1) << 8)
                                   | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2])) << 4
                                   | (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 4;
...
//      		  exit(0);
			  ...
			  xs->tag = tree(bucketid, s0, s1);
                ...
                if(htl.hta.heap0[xorbucketid][xorslot]->bytes[0] == 0x0 && htl.hta.heap0[xorbucketid][xorslot]->bytes[1] == 0x0 && htl.hta.heap0[xorbucketid][xorslot]->bytes[2] == 0x2b && htl.hta.heap0[xorbucketid][xorslot]->bytes[3] == 0xcc && htl.hta.heap0[xorbucketid][xorslot]->bytes[4] == 0x92){
                //if(xorbucketid == 6830 && xorslot == 32){
                	printf("xhash1: %d\n", xhash1);
                	for(int i = 0; i < 24; i++){
                		printf("%.2x ", slot0->bytes[i]);
                	}
                ...
                //printf("prevbo: %d\n", htl.prevbo);
//                for(int i = 0; i < 12; i ++){
//                	printf("%.2x ", htl.hta.heap0[xorbucketid][xorslot]->bytes[i]);
//                }
//                printf("\n");
...
//				const htunit *slot1 = buck[s1];
//                //if(i == 6830 && s1 == 32){
//					 printf("bucket: %d, slot: %d  ", i, s1);
//					 for(int j=0; j < 16; j++){
//						printf("%.2x ", slot1->bytes[j]);
//					}
//					 printf("\n");
//                //}
//			}
...
//			  printf("\n");
              ...
              xorbucketid = (((u32)(bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) & 0x7f) << 6)
            				| (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 2;
...
//			exit(0);
            ...
            for (; cd.nextcollision(); ) {
              ...
              xorbucketid = ((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x3) << 8)
                                | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2])) << 3
                                | (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 5;
...
//			  exit(0);
    ...
    void digit8(const u32 id) {
        ...
        /*
		for(int i = id; i < NBUCKETS; i++){
			slot1 *buck = htl.hta.heap1[i]; // point to first slot of this bucket
			u32 bsize   = getnslots1(i);    // grab and reset bucket size
			for (u32 s1 = 0; s1 < bsize; s1++) {   // loop over slots
				const htunit *slot1 = buck[s1];
				//if(i == 6830 && s1 == 32){
					 printf("bucket: %d, slot: %d  ", i, s1);
...
//            printf("xhash1: %d prevbo: %d\n\n", xhash1, htl.prevbo);
            ...
            for (; cd.nextcollision(); ) {
              ...
              xorbucketid = ((u32)(bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) << 5)
                                | (bytes0[htl.prevbo+3] ^ bytes1[htl.prevbo+3]) >> 3;
...
//		  exit(0);
          ...
          for (; cd.nextcollision(); ) {
            const u32 s0 = cd.slot();
            const htunit *slot0 = buck[s0];
            // there is only 1 word of hash left - NOT ANYMORE
			...
			if(s0_1 == s1_1 &&
			   ...
			   s0_4 == s1_4){
				...
				printf("s0_1: %d, s1_1: %d\n", s0_1, s1_1);
				printf("s0_2: %d, s1_2: %d\n", s0_2, s1_2);
				printf("s0_3: %d, s1_3: %d\n", s0_3, s1_3);
				printf("s0_4: %d, s1_4: %d\n\n", s0_4, s1_4);
...
//			//exit(0);
//
//			printf("first %x %x %x \n", slot1->bytes[htl.prevbo], slot1->bytes[2], slot1->bytes[3]);
//			*/
//			u32 xhash0 = ((slot1->bytes[htl.prevbo] & 0x7f) << 1) | (slot1->bytes[htl.prevbo+1] >> 7);
//
//			cd.addslot(s1, xhash0);// identify list of previous colliding slots
//			//printf("xhash0: %d\n", xhash0);
//			//exit(0);
...
//
//
//			  xorbucketid = ((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x7f) << 6)
//								   | (bytes0[htl.prevbo+2] ^ bytes1[htl.prevbo+2]) >> 2);
//
//
////          xorbucketid =((((u32)(bytes0[htl.prevbo+1] ^ bytes1[htl.prevbo+1]) & 0x1) << 8)
...
//  		  */
//
//  		  u32 xhash0 = (slot1->bytes[htl.prevbo] & 0x1) | (slot1->bytes[htl.prevbo+1] >> 7) ;
//
//  		  cd.addslot(s1, xhash0);// identify list of previous colliding slots
//  		  //exit(0);
//  		  for (; cd.nextcollision(); ) {
...
//
//		  /*
//		  if(slot1->bytes[htl.prevbo] == 0x0c && slot1->bytes[htl.prevbo + 1] == 0x42 && slot1->bytes[htl.prevbo + 2] == 0x47){
//		  printf("prevbo: %d\n", htl.prevbo);
//			  for(int i = htl.prevbo; i <htl.prevbo + 24; i++){
//				printf("%x ", slot1->bytes[i]);
//			  }
//			  printf("\n");
//			  exit(0);
...
//
//
//		  	  if(slot0->bytes[htl.prevbo] == 0x05 && slot0->bytes[htl.prevbo + 1] == 0x92 && slot0->bytes[htl.prevbo + 2] == 0x94){
//				printf("prevbo: %d\n", htl.prevbo);
//				  for(int i = htl.prevbo; i  < 24 + htl.prevbo; i++){
//					  printf("%x ", bytes0[i]);
//				  }
//				  printf("\n");
//				  for(int i = htl.prevbo; i < 24 + htl.prevbo; i++){
...
//		  	 if(slot0->bytes[htl.prevbo] == 0x05 && slot0->bytes[htl.prevbo + 1] == 0x92 && slot0->bytes[htl.prevbo + 2] == 0x94){
//				for(int i = 0; i < 25; i++){
//					printf("%x ", htl.hta.heap1[xorbucketid][xorslot]->bytes[i]);
//				}
//				printf("\n");
//				//exit(0);
//		  	 }
...
//	        const htunit *slot1 = buck[s1];
//
//	        //if(slot1->bytes[0] == 0x3 && slot1->bytes[1] == 0xae && slot1->bytes[2] == 0xcd){
//	            for(int i = 0; i < 24; i++){
//	            	printf("%.2x ", slot1->bytes[i]);
//	            }
//	            printf("\n");
